...
$m->properties['Bewohner1'] = 10; $m->properties['Bewohner1Image'] = 30;
$m->ApplyChanges();
...
$cache = $m->buffers['ResidentThumbnail1'];
$m->ApplyChanges();
check($cache === $m->buffers['ResidentThumbnail1'] && !isset(latest($m)['image1']), 'Cached thumbnail reused with delta updates');
...
check($cache !== $m->buffers['ResidentThumbnail1'], 'Media content update invalidates thumbnail cache');
...
for ($i=1;$i<=5;$i++) { $m->properties['Bewohner'.$i]=10; $m->properties['Bewohner'.$i.'Image']=30; }
